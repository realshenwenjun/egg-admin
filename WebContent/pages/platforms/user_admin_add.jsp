        ...
        <ul class="clearfix">
        	<c:forEach items="${list}" var="user">
				<li class="clearfix teatcher">
                    ...
                    <a name="addAdmin" key="${user.id }" href="javaScript:" class="blue">确认添加</a>
				</li>
			</c:forEach>
          </ul>
        </div>
...
<script type="text/javascript">
	...
	$(function(){
		...
		$("input[name='searchAdmin']").change(function(){
	       ...
	       $.ajax({ 
			type : "GET",
			url : "/admin/user/nomal/search.do?key="+key,
			dataType : "html",
    	...
    	$("a[name='addAdmin']").click(function(){
    		var addAdmin = $(this);
			var userId = addAdmin.attr("key");
	       $.ajax({
			type : "GET",
			url : "/admin/user/supper/admin/add.do?userId="+userId,
			dataType : "json",
			success : function(data) {
				if(data.success){
					addAdmin.parent("li").remove();
				}else{
					alert(data.result);
				}
			   }
