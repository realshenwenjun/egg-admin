...
<script type="text/javascript">
	...
	window.editor = K.create('textarea[name="context"]',options);
		...
		$("a[name='updateInformation']").click(function() {
			...
			$.ajax({
				type : "POST",
				url : "/admin/information/update.do",
				data : {
					id : "${information.id}",
					path : path,
					title : title,
...
<body>
	<div class="w1200">
		<div class="waper fr">
			<h3 class="big-title">编辑资讯</h3>

			<h3 class="small-title">编辑图文消息</h3>
			<div class="inpour clearfix">
				<label class="w95 fl">标题：</label>
				<input type="text" class="inputst w565 fl" name="title" value="${information.title }" placeholder="">
			</div>
			<h3 class="title">封面图片</h3>

			<div class="img-all">
				<div id="load_img" class="imgs fl" >
					<img alt="" src="${information.imgUrls[0] }" width="200" height="150">
				</div>
				<div class="texts fl">
				...
				<input type="hidden" name="path" value="${information.imgUrls[0] }"/>
			</div>
			<h3 class="title">摘要</h3>
			<div class="kindeditors">
				<textarea name="summary" width="1024" value="${information.summary }">${information.summary }</textarea>
			</div>
			<h3 class="title">正文</h3>
			<div class="kindeditors">
				<textarea name="context" style="width:1000px;height:300px;">${information.context }</textarea>
			</div>
			<div class="btns-paper">
				<a href="javaScript:" name="updateInformation" class="submit">保存</a>
			</div>
		</div>
	</div>
</body>
